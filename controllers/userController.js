...
class UserController {
    async registration(req, res, next) {
        ...
        if (errorCollector.length !== 0) {
            ...
            return (next(ApiError.internal(`Some fields are already exists: ${errorCollector}`)));
        };
    ...
    async login(req, res, next) {
        ...
        if (login) {
            ...
            if (!candidate) {
                return next(ApiError.internal(`Error! This LOGIN doesn't exist.`));
            };
        } else if (email) {
            candidate = await user.findOne({ where: { email } });
            if (!candidate) {
                return next(ApiError.internal(`Error! This EMAIL doesn't exist.`));
            };
        } else {
            return next(ApiError.internal(`Error! Server can't find "LOGIN" or "EMAIL".`));
        };
